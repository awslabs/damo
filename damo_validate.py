...
def check_boundary(region, regions_boundary):
    ...
    for boundary in regions_boundary:
        if (region.start >= boundary[0] and region.end <= boundary[1]):
            in_boundary = True
            break
    if not in_boundary:
        print('region %s-%s out of the boundary' % (region.start, region.end))
        exit(1)
...
def set_argparser(parser):
    ...
    parser.add_argument('--nr_regions',
                        metavar='<number of regions>',
                        type=int,
                        nargs=2,
                        default=[3, 1200],
    ...
    parser.add_argument(
        '--nr_accesses',
        metavar='<number of accesses>',
        type=int,
        nargs=2,
        default=[0, 24],
        help='min/max number of measured accesses per aggregate interval')
    parser.add_argument('--allow_error',
                        metavar='<percent>',
                        default=2,
                        help='allowed percent of error samples')
    parser.add_argument('--regions_boundary',
                        metavar='<start>-<end>',
                        nargs='+',
                        help='regions boundary')
...
def main(args=None):
    ...
    for record in records:
        ...
        for snapshot in record.snapshots:
            ...
            nr_aggr_interval_errors += assert_value_in_range(
                aggr_interval_us, args.aggr, 'aggregate interval',
                nr_aggr_interval_errors < nr_allowed_errors)

            nr_nr_regions_erros += assert_value_in_range(
                len(snapshot.regions), args.nr_regions, 'nr_regions',
