...
class TestDamoSchemesInput(unittest.TestCase):
    def test_schemes_option_to_damos(self):
        ...
        expected_damos_wo_filters = [
            _damon.Damos(
                _damon.DamosAccessPattern([0, 0], [0, 0], _damon.unit_samples,
                                          [0, 0], _damon.unit_aggr_intervals),
                'stat', _damon.DamosQuotas(0, 0, 0, [0, 0, 0]),
        ...
        expected_damos_w_filters = [
            _damon.Damos(
                ...
                _damon.DamosWatermarks('none', 0, 0, 0, 0), [
                    ...
                    _damon.DamosFilter('memcg', '/all/latency-critical', False)
                ], None, None)
        ]

        def get_damos_from_damo_schemes(damo_schemes):
            damos, err = _damon_args.schemes_option_to_damos(damo_schemes)
            return damos

        _test_damo_common.test_input_expects(
            self, get_damos_from_damo_schemes, {
                ...
                human_readable_damos_with_filters_str: expected_damos_w_filters
            })
...
2M max      min min     7s max      nohugepage''')
        ...
        self.assertEqual(damos_list, [
            _damon.Damos(access_pattern=_damon.DamosAccessPattern(
                sz_bytes=['min', 'max'],
                nr_accesses=['5', 'max'],
