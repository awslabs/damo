...
class TestDamon(unittest.TestCase):
    def test_kvpairs_transition(self):
        ...
        self.assertEqual(
            list(damos_kvpairs.keys()),
            ['action', 'access_pattern', 'quotas', 'watermarks', 'filters'])
        self.assertEqual(damos, _damon.Damos.from_kvpairs(damos_kvpairs))

        ctx = _damon.DamonCtx(_damon.DamonIntervals(5000, 100000, 1000000),
        ...
        self.assertEqual(
            list(ctx_kvpairs.keys()),
            ['intervals', 'nr_regions', 'ops', 'targets', 'schemes'])
        ...
        self.assertEqual(list(kdamond_kvpairs.keys()),
                         ['state', 'pid', 'contexts'])
        self.assertEqual(kdamond, _damon.Kdamond.from_kvpairs(kdamond_kvpairs))
    ...
    def test_damos_eq(self):
        damos = _damon.Damos(access_pattern=_damon.DamosAccessPattern(
            [4096, 18446744073709551615], [0.0, 0.0], _damon.unit_percent,
        ...
        damos2.filters = [
            _damon.DamosFilter(filter_type='memcg',
                               memcg_path='/foo/bar/',
                               matching=True)
        ]
        ...
        pattern_human = _damon.DamosAccessPattern([123, 456], [15, 35],
                                                  _damon.unit_percent,
                                                  [5000000, 19000000],
                                                  _damon.unit_usec)
        pattern_machine = _damon.DamosAccessPattern([123, 456], [3, 7],
                                                    _damon.unit_samples,
                                                    [50, 190],
    ...
    def test_damon_intervals(self):
        ...
        _test_damo_common.test_input_expects(
            self, lambda x: _damon.DamonIntervals.from_kvpairs(json.loads(x)), {
                json.dumps({
                    'sample_us': 5000,
                    'aggr_us': 100000,
        ...
        self.assertEqual(
            '%s' % _damon.DamonIntervals(5000, 100000, 1000000).to_str(True),
        ...
        self.assertEqual(
            _damon.DamonIntervals(5000, 100000, 1000000).to_kvpairs(raw=True), {
                'sample_us': '5000',
                'aggr_us': '100000',
                'ops_update_us': '1000000'
            })
    ...
    def test_damon_nr_regions_range(self):
        ...
        _test_damo_common.test_input_expects(
            self, lambda x: _damon.DamonNrRegionsRange(*x), {
        ...
        _test_damo_common.test_input_expects(
            self,
            lambda x: _damon.DamonNrRegionsRange.from_kvpairs(json.loads(x)), {
                json.dumps({
                    'min': 10,
                    'max': 1000
                }): expect,
        ...
        self.assertEqual(
            _damon.DamonNrRegionsRange(10, 1000).to_kvpairs(), {
                'min': '10',
                'max': '1,000'
            })
        self.assertEqual(
            _damon.DamonNrRegionsRange(10, 1000).to_kvpairs(raw=True), {
                'min': '10',
                'max': '1000'
            })
    ...
    def test_damon_region(self):
        _test_damo_common.test_input_expects(
            self, lambda x: _damon.DamonRegion(*x), {
        ...
        _test_damo_common.test_input_expects(
            self, lambda x: _damon.DamonRegion.from_kvpairs(json.loads(x)), {
                json.dumps({
                    'start': '123',
                    'end': '456'
                }):
        ...
        self.assertEqual(
            _damon.DamonRegion(1234, 5678).to_kvpairs(), {
                'start': '1,234',
                'end': '5,678'
            })
        self.assertEqual(
            _damon.DamonRegion(1234, 5678).to_kvpairs(raw=True), {
                'start': '1234',
                'end': '5678'
            })
    ...
    def test_damos_access_pattern(self):
        self.assertEqual(
            ...
            _damon.DamosAccessPattern(['min', 'max'], ['min', 'max'],
                                      _damon.unit_percent, ['min', 'max'],
                                      _damon.unit_usec))
        ...
        pattern_human = _damon.DamosAccessPattern([123, '4,567'],
                                                  ...
                                                  _damon.unit_usec)
        pattern_machine = _damon.DamosAccessPattern([123, 4567], [3, '2,000'],
                                                    _damon.unit_samples,
                                                    [50, '1,900'],
                                                    _damon.unit_aggr_intervals)
        ...
        self.assertEqual(
            pattern_human.converted_for_units(_damon.unit_samples,
                                              _damon.unit_aggr_intervals,
                                              intervals), pattern_machine)
        self.assertEqual(
            pattern_machine.converted_for_units(_damon.unit_percent,
                                                _damon.unit_usec, intervals),
            pattern_human)
        ...
        exception_raised = False
        try:
            _damon.DamosAccessPattern(nr_accesses=[0, 100],
                                      nr_accesses_unit='foo')
        except:
            exception_raised = True
        self.assertTrue(exception_raised)
        exception_raised = False
    ...
    def test_damos_watermarks(self):
        self.assertEqual(
            _damon.DamosWatermarks(),
            _damon.DamosWatermarks(_damon.damos_wmarks_metric_none, 0, 0, 0, 0))
