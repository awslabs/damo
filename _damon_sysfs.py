...
def __ensure_scheme_dir_populated(scheme_dir, scheme):
    ...
    nr_filters_path = os.path.join(scheme_dir, 'nr_filters')
...
def __ensure_target_dir_populated(target_dir, target):
    nr_regions_path = os.path.join(target_dir, 'nr_regions')
...
def __ensure_kdamond_dir_populated(kdamonds, kdamond):
    ...
    for idx, ctx in enumerate(kdamond.contexts):
        ...
        for target_idx, target in enumerate(ctx.targets):
            target_dir = target_dir_of(kdamond_name, ctx_dir_name,
                    '%d' % target_idx)
            __ensure_target_dir_populated(target_dir, target)
        nr_schemes, err = _damo_fs.read_file(
                nr_schemes_file_of(kdamond_name, ctx_dir_name))
        if err != None:
            raise Exception('nr_schemes read fail (%s)' % err)
        if int(nr_schemes) != len(ctx.schemes):
            _damo_fs.write_file(nr_schemes_file_of(kdamond_name, ctx_dir_name),
                    '%d' % len(ctx.schemes))
        for scheme_idx, scheme in enumerate(ctx.schemes):
            scheme_dir = scheme_dir_of(
                    kdamond_name, ctx_dir_name, '%s' % scheme_idx)
            __ensure_scheme_dir_populated(scheme_dir, scheme)
...
def __ensure_dirs_populated_for(kdamonds):
    ...
    for kdamond in kdamonds:
        __ensure_kdamond_dir_populated(kdamonds, kdamond)
