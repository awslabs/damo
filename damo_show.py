...
def format_field(min_chars, field_name, index, region, snapshot, record, raw):
    if field_name == '<index>':
        ...
        if snapshot.total_bytes == None:
            snapshot.total_bytes = sum(
                [r.end - r.start for r in snapshot.regions])
        ...
        txt = _damo_fmt_str.format_time_ns(snapshot.start_time - base_time, raw)
...
def format_pretty(template, min_chars, index, region, snapshot, record, raw):
    for field_name in [
            ...
            '<monitor start time>',
            '<monitor end time>',
            '<monitor start abs time>',
            '<monitor end abs time>',
            # for record pretty
            '<kdamond index>',
            '<context index>',
            '<scheme index>',
        ...
        template = template.replace(
            field_name,
            format_field(min_chars, field_name, index, region, snapshot, record,
                         raw))
    template = template.replace('\\n', '\n')
    return template
...
def pr_records(args, records):
    ...
    for record in records:
        if args.record_head_pretty != '':
            print(
                format_pretty(args.record_head_pretty, args.pretty_min_chars,
        ...
        for sidx, snapshot in enumerate(snapshots):
            if args.snapshot_head_pretty:
                print(
                    format_pretty(args.snapshot_head_pretty,
                                  args.pretty_min_chars, None, None, snapshot,
            ...
            for idx, r in enumerate(snapshot.regions):
                ...
                print(
                    format_pretty(args.region_pretty, args.pretty_min_chars,
                                  idx, r, snapshot, record, args.raw_number))
            if args.snapshot_tail_pretty:
                print(
                    format_pretty(args.snapshot_tail_pretty,
                                  args.pretty_min_chars, None, None, snapshot,
                                  record, args.raw_number))
        ...
        if args.record_tail_pretty != None:
            print(
                format_pretty(args.record_tail_pretty, args.pretty_min_chars,
                              None, None, None, record, args.raw_number))
...
def set_argparser(parser):
    ...
    parser.add_argument('--tried_regions_of',
                        nargs=3,
                        type=int,
                        action='append',
                        metavar=('<kdamond idx>', '<context idx>',
                                 '<scheme idx>'),
    ...
    parser.add_argument(
        '--region_pretty',
        default=
        '<index> addr [<start address>, <end address>) (<region size>) access <access rate> age <age>',
        help='region output format')
    parser.add_argument('--pretty_min_chars',
                        nargs=2,
                        default=[],
                        metavar=('<field name> <number>'),
                        action='append',
                        help='minimum character for each field')
    parser.add_argument('--total_sz_only',
                        action='store_true',
...
def main(args=None):
    ...
    access_pattern = _damon.DamosAccessPattern(args.sz_region, args.access_rate,
                                               _damon.unit_percent,
                                               args.age * 1000000,
                                               _damon.unit_usec)
    ...
    if args.input_file == None:
        ...
        records, err = _damon_result.get_snapshot_records(
            access_pattern, args.total_sz_only)
        if err != None:
            print(err)
            exit(1)
    else:
